...
class PdfFile(
     ...
     */
    private val spacingTopPx: Int,
    /**
     * Fixed spacing in bottom of first page
     */
    private val spacingBottomPx: Int
) {
    ...
    fun getLinkAtPos(currentPage: Int, posX: Float, posY: Float): Long {
        return pdfiumCore.nativeGetLinkAtCoord(
            pdfDocument.mNativePagesPtr[currentPage]!!, size.width.toDouble(),
    ...
    fun mapRectToDevice(
        ...
        return pdfiumCore.mapRectToDevice(pdfDocument, mPage, startX, startY, sizeX, sizeY, 0, rect)
    }
    ...
    private fun prepareDocLen() {
        ...
        val mSpacing: Int = spacingPx * (mScaledPageSizes.size - 1) + spacingTopPx + spacingBottomPx
        mDocumentLength = mLength + mSpacing
    }

    private fun preparePagesOffset() {
        mPageOffsets.clear()
        var mOffset = spacingTopPx.toFloat()
        for (i in 0 until pagesCount) {
            val mPageSize: SizeF = mScaledPageSizes[i]
    ...
    fun renderPageBitmap(bitmap: Bitmap, pageIndex: Int, bounds: Rect, annotation: Boolean) {
        ...
        pdfiumCore.renderPageBitmap(
            pdfDocument, bitmap, docPage, bounds.left, bounds.top, bounds.width(),
            bounds.height(), annotation
        )
    }
