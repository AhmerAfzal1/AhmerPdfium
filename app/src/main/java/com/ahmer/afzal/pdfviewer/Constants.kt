package com.ahmer.afzal.pdfviewer

class Constants {

    companion object {
        const val DATA_STORE_NAME: String = "PdfiumPref"
        const val TAG: String = "AhmerPdf"
        const val PDF_FILE: String = "files"
        const val PDF_FILE_MAIN: String = "main"
        const val PDF_FILE_1: String = "file1"
        const val PDF_FILE_2: String = "file2"
        const val PDF_FILE_3: String = "file3"
        const val PDF_FILE_4: String = "file4"
        const val PDF_FILE_PROTECTED: String = "protected"
        const val PDF_SAMPLE_FILE: String = "statement.pdf"
        const val PDF_SAMPLE_FILE_PASSWORD_PROTECTED: String = "proverbs.pdf"
        const val PDF_URI = "pdf_uri"
        const val PDF_FILE_SD = "pdf_file_sd"


    }
}